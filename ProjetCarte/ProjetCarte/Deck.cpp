...
{
	int hand_counter = 0;
	for (int i = (Const_var::nmbr_deck_cards -1) ; i > 0; --i) // On regarde toute les cartes du paquet et on va en soustraire jusqu'a 10 pour les ajouter dans la main du joueur
	{
		if ( (this->tab_Cards[i]->In_Deck() == true) && hand_counter < Const_var::nmbr_cards_in_Hand)
		{
...
void Deck::Mix_card()// Reprend les 104 cartes du jeux et les mellange
{
	...
	for (int i = 0; i < Const_var::nmbr_deck_cards; i++) tampon_deck_cards[i] = nullptr; // On met toute les valeurs à 0 car par defaut les valeurs d'un tableau créé statiquement en C++ sont inconnue
	for (int i = 0; i < Const_var::nmbr_deck_cards; i++)
	{ 
	...
	for (int i = 0; i < Const_var::nmbr_deck_cards; i++) tab_Cards[i] = tampon_deck_cards[i];
}
