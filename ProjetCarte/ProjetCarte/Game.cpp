		...
		for (int i = 0; i < Const_Var::nmbr_cards_in_Hand && (end == false); i++)
		{
			...
			for (int j = 0; j < Const_Var::nmbr_Gamer; j++)
			{
				if (les_joueurs[j]->Get_score() >= 66)
				{
					...
					cout << "\n\n--------------------------------------[ C'est le joueur " << (les_joueurs[j]->Get_number() + 1 ) << " Qui prend !! ]--------------------------------------";
					Sql_score_insertion(les_joueurs);
					break;
				}
			}
		}
...
{
	int z = 0;
	sqlite3 *db; // On cré notre objet sqlite3 qui representera notre Base de donnée qu'on liera par la suite
	char *zErrMsg = 0;
